...
// Functional component for displaying top artists
const TopArtist = () => {
  const [artist, setArtist] = useState();

  // useEffect hook to fetch top artists when the component mounts
  useEffect(() => {
    if (!artist || artist.length === 0) {
      axios.get('http://127.0.0.1:5000/top_artist') // Fetch top artists from the server
        .then(response => {
