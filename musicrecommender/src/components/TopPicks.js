...
// Functional component for displaying top picks
const TopPicks = () => {
  const [track, setTrack] = useState();

  // useEffect hook to fetch top tracks when the component mounts or 'track' state changes
  useEffect(() => {
    if (!track || track.length === 0) {
      axios.get('http://127.0.0.1:5000/top_track') // Fetch top tracks from the server
        .then(response => {
